{if $slides|@count != 0}
    {if $minicSlider.options.front == 1 && $page_name != 'index'}
        <!-- Minic Slider -->
    {else}
        <div id="minic_slider" class="theme-default span8{if $minicSlider.options.thumbnail == 1 and $minicSlider.options.control != 0} controlnav-thumbs{/if}">   
            ...
            {foreach from=$slides item=caption name=singlecaption}
                {if $caption.title != '' or $caption.caption != ''}
                    <div id="htmlcaption_{$caption.id_slide}" class="nivo-html-caption">
                        <h2>{$caption.title}</h2>
                        <a class="minic_caption minic_caption-{$smarty.foreach.singlecaption.iteration}" href="{$image.url}">{$caption.caption}</a>
                    </div>
                {/if}
            {/foreach}
        </div> 
